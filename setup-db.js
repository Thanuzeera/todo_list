...
async function setupDatabase() {
  ...
  // Read the SQL file
  const sqlFile = path.join(__dirname, 'backend', 'database.sql');
  ...
  for (const password of commonPasswords) {
    try {
      ...
      envContent = envContent.replace('your_password_here', password);
      fs.writeFileSync(envPath, envContent);
      
      console.log(`\n🔧 Updated backend/.env with password: ${password || '(empty)'}`);
  ...
  console.log('4. Update backend/.env with your actual password');
}
